...
SELECT ASSUNTO.ASSUNTO AS ASSUNTO, COUNT(ASSUNTO) AS QUANTIDADE FROM ASSUNTO
    JOIN ASSUNTOPOST ON ASSUNTOPOST.CODIGOASSUNTO = ASSUNTO.CODIGO
    JOIN POST ON ASSUNTOPOST.CODIGOPOST = POST.CODIGO
    JOIN COMPARTILHAMENTO ON COMPARTILHAMENTO.COD_POST = POST.CODIGO
WHERE POST.EMAIL_USUARIO = 'pmartinssilva90@mymail.com' AND
COMPARTILHAMENTO.EMAIL_USUARIO = 'joaosbras@mymail.com' AND 
COMPARTILHAMENTO.DATACOMPARTILHAMENTO BETWEEN date ('now', '-3 months', 'localtime' ) 
    AND DATE ('now');
